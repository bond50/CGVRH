...
const CoreValues = () => {
    const list = [
        ...
        },
        {
            ...
            para: 'To provide specialized integrated client centered health services through training, research and collaboration to improve quality of lives'
        },
        {
            title: 'Core values',
        ...
        return list.map((item, index) => {
            if (item.title === 'Core values') {
                ...
                const coreValuesList = item.para.split(',').map((value, i) => (
                    <li key={i}>
                        <Icon icon="ri:check-double-line" className={styles.ico1} /> {value.trim()}
                    </li>
                ));
                ...
                return (
                    ...
                    >
                        <div className={`w-100 ${styles.iconBox}`}>
                            <div className={styles.icon}>
                                <Icon icon={item.icon} className={styles.ico} />
                            </div>
                            <h4>{item.title}</h4>
